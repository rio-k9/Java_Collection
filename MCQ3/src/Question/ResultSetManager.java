...
/**
* ResultSetManager
* <p>A class for the management of ResultSet objects.</p>
* @author  Adam Kearney
*/
...
public class ResultSetManager {
	...
	public static HashMap resultSetToMap(ResultSet input) throws SQLException {
	...
	public static HashMap resultSetToMap(ResultSet input, int key, int[] values) throws SQLException {
	...
	private static HashMap _resultSetToMap(ResultSet input) throws SQLException {
		...
		while (input.next()) {
			...
			//Create the array
			Object[] valuesArray = new Object[columnCount];
			...
			for (int i = 1; i<=columnCount; i++) {
				...
				if (columnTypeName.equals("int")) {
					valuesArray[i-1] = input.getInt(columnName);
				}
	...
	private static HashMap _resultSetToMap(ResultSet input, int key, int[] values) throws SQLException {
		...
		while (input.next()) {
			...
			for (Integer i = 1; i<=columnCount; i++) {
				...
				if (test) {
					...
					//Writing this out the long way, but still probably better than using reflection
					if (columnTypeName.equals("int")) {
						valuesArray[j] = input.getInt(columnName);
						j++;
					}
					else if (columnTypeName.equals("String")) {
	...
	private static String _columnDataTypeChecker(int SQLType) {
		...
		switch ( SQLType ) {
			...
			case 0:  JavaType = "null"; break; //null type; this is not the same as the column being empty, which should not be allowed to happen
